...
void loop() {
  ...
  switch (Command) {
    ...
    case 'S':  // Stop current action
      ...
      break;
    case 'd':
    case 'D':  // Drive in straight line using encoders
      if (PREDRIVING) {
        //Direction
        distAngle = ((direction == 'f') || (direction == 'F')) ? (distAngle) : (-distAngle);
        // Zero
        leftEnc.write(0);
        rightEnc.write(0);
        elapsed_distance = 0;
        ...
        theta2_final = distAngle / rw;
        omega1_des = fabs(theta1_final * speedRadius / distAngle) * (distAngle > 0 ? 1 : -1);
        omega2_des = fabs(theta2_final * speedRadius / distAngle) * (distAngle > 0 ? 1 : -1);
        t_old = 0.;
        t0 = micros() / 1000000.;
        // Bools
        PREDRIVING = false;
      ...
      if (DRIVING) {
        driveDistance(distAngle, DEBUGMODE);
      } else {
        Command = 0;
        md.setSpeeds(0, 0);
        PREDRIVING = true;
    ...
    case 'T':  // Turn, either in an arc or in place
      if (PREDRIVING) {
        //Direction
        ISARC = (speedRadius == 0) ? false : true;
        // Zero
        leftEnc.write(0);
        rightEnc.write(0);
        t0 = micros() / 1000000.;
        ...
        // Calculate Trajectories
        if (ISARC) {
          double desiredR = speedRadius;
          double desiredPhi = distAngle / 180. * PI;
          switch (direction) {
            case 'r':
            case 'R':
      ...
      if (DRIVING) {
        turnDeg(ISARC, speedRadius, distAngle, DEBUGMODE);
      } else {
        Command = 0;
        md.setSpeeds(0, 0);
        PREDRIVING = true;
    ...
    case 'L':  // Line follow F/R
      switch (direction) {
        ...
        case 'F':
          ...
          if (DRIVING && (elapsed_distance <= distAngle)) {
            elapsed_distance = lineFollow('f');
          } else {
            Command = 0;
            md.setSpeeds(0, 0);
            DRIVING = false;
