...
int MineDelay = 160;  // Amount of time to wait between mining strokes
int mineStrokes;
const int mineServoRest = 45;
const int mine1 = 175;
const int mine2 = 142;
const int towerServoRest = 40;
const int towerServoStart = 0;
const int towerServoTap = 70;
const int towerServoBlock = 110;
int loadDelay = 800;
...
float MineApproachStop = 3.2;  // Stop distance from the wall (cm)
...
float CraftApproachSlow = 12;  // Distance at which you start slowing at wall approach.
  ...
  switch (MetaState) {
    ...
    case 6:
      ...
      if (numBlocks == 0 && !DONEWITHCOMMAND) {  // Out of blocks, not done crafting
        if (goal == 'S') {
          CraftApproachSlow = 15;
          CraftApproachStop = 9.5;
        } else if (goal == 'P') {
          CraftApproachStop = 4;
