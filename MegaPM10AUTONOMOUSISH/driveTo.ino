  ...
  switch (position) {                                  // Switch on the current position
    case 'S':                                          // Start
      if (destination == 'M' || destination == 'm') {  // Drive to Mine from Start
        position = lineFollow('B', 0, autonomous_speed) ? position : 'M';
        DONEDrive = (position == 'M') ? true : false;
        if (DONEDrive) drive_to_substate = 0;
      }
      break;
    case 'M':                                          // Mine
      if (destination == 't' || destination == 'T') {  // Drive to Tree from Mine
        if (drive_to_substate == 0) {
          ...
          if (lineFollow('R', 0, autonomous_speed)) {
            drive_to_substate = 0;
            DONEDrive = true;
            position = 'T';
          }
        ...
        if (drive_to_substate = 0) {
          ...
          if (lineFollow('F', 0, autonomous_speed)) {
            drive_to_substate = 0;
            DONEDrive = true;
            position = 'C';
          }
        ...
        if (drive_to_substate == 0) {
          ...
          if (lineFollow('F', 10, autonomous_speed)) drive_to_substate = 3;
        } else if (drive_to_substate == 4) {
      ...
      if (destination == 'm' || destination == 'M') {  // Drive to Mine from Tree
        if (drive_to_substate == 0) {
          ...
          if (drive_to_substate == 0) {
            ...
            if (driveDistance('F', 61, autonomous_speed, DEBUGMODE)) drive_to_substate = 2;
          } else if (drive_to_substate == 2) {
            if (turnDeg('R', 90, 0, DEBUGMODE)) drive_to_substate = 3;
          } else if (drive_to_substate == 3) {
            if (lineFollow('F', 0, autonomous_speed)) {
