...
void setup() {
  ...
  // Set Mining Servo to resting position
  MiningServo.write(45, 255, true);

  // Set Tower Servo to resting position
  TowerServo.write(90, 255, false);

  // Set Dispense Servo to resting position

  if (servo) {
    Serial.println("Enter Servo name followed by 3 digits of position (ie 020 or 160)");
    Serial.println("You can also simply type s or S to stop");
    Serial.println("Servo Options: ");
    Serial.println("M <deg> : Mining Servo");
    Serial.println("T <deg> : Tower Servo");
    Serial.println("D <deg> : Dispense Servo");
  }
}
...
void loop() {
  ...
  if (servo) {
    ...
    switch (command) {
      ...
      case 'd':
      case 'D':
        DispenseServo.write(angle, servoSpeed, false);
        command = 0;
        break;
    }
  }
